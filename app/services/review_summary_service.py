...
class ReviewSummaryService:
    ...
    def _document_selected_evidence(self, summary: dict[str, Any]) -> list[dict[str, Any]]:
        selected_achievements = [
            item
            for item in list(summary.get("selected_achievements") or [])
            if isinstance(item, dict)
        ]
        selected_evidence_ids = [
            str(item or "").strip()
            for item in list(summary.get("selected_evidence_ids") or [])
            if str(item or "").strip()
        ]
        evidence_reasons = [
            item
            for item in list(summary.get("evidence_selection_reason") or [])
            if isinstance(item, dict)
        ]
        ...
        for evidence_id in selected_evidence_ids:
            achievement = evidence_by_id.get(evidence_id, {})
            append_item(
        ...
        for reason_item in evidence_reasons:
