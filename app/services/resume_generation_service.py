...
# Доля токенов оригинала, которую AI-улучшение может потерять при
# перефразировании без отбрасывания gate'ом. 0.3 было слишком строго для
# русского текста — нормальное перефразирование (синонимы, перестановки)
# теряет >30% слов-токенов, и gate молча возвращал оригинал («нет отличий»).
# 0.5 — допускает сильное перефразирование, но отбрасывает полную подмену
# содержимого; protected_terms (PROTECTED_TECH_TERMS) и factuality-gate
# (метрики) отдельно защищают факты.
MAX_KEYWORD_LOSS_RATIO = 0.5
...
class ResumeGenerationService:
    ...
    def _normalize_text_tokens(self, text: str) -> set[str]:
        ...
        stopwords = {
            ...
            "system",
            "built",
            # Русские служебные/связочные (>= 4 символов) — не несут фактов,
            # AI естественно их заменяет при перефразировании; исключение из
            # расчёта retention повышает пропускную способность gate для RU.
            "которые",
            "которая",
            "которое",
            "которых",
