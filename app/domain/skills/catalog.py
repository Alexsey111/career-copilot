# app\domain\skills\catalog.py

from __future__ import annotations

from dataclasses import dataclass


@dataclass(frozen=True)
class SkillDefinition:
    canonical_name: str
    patterns: list[str]
    related_skills: list[str]


SKILL_DEFINITIONS: list[SkillDefinition] = [
    SkillDefinition(
        canonical_name="Python",
        patterns=[r"\bpython\b", r"\bпитон\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="SQL",
        patterns=[
            r"\bsql\b",
            r"\bсубд\b",
            r"баз[аы]\s+данных",
        ],
        related_skills=["PostgreSQL"],
    ),
    SkillDefinition(
        canonical_name="API",
        patterns=[
            r"\bapi\b",
            r"\brest\b",
            r"rest\s*api",
            r"интеграц\w*\s+с\s+api",
        ],
        related_skills=["FastAPI"],
    ),
    SkillDefinition(
        canonical_name="FastAPI",
        patterns=[
            r"\bfastapi\b",
            r"fast\s*api",
        ],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="PostgreSQL",
        patterns=[
            r"\bpostgres(?:ql)?\b",
            r"\bpostgresql\b",
        ],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Git",
        patterns=[r"\bgit\b", r"\bgithub\b", r"\bgitlab\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Redis",
        patterns=[r"\bredis\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Docker",
        patterns=[
            r"\bdocker\b",
            r"\bконтейнеризац",
        ],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="SQLAlchemy",
        patterns=[r"\bsqlalchemy\b", r"sql\s*alchemy"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Alembic",
        patterns=[r"\balembic\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Pydantic",
        patterns=[r"\bpydantic\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Pytest",
        patterns=[
            r"\bpytest\b",
            r"\bunit\s+tests?\b",
            r"\bтестировани[ея]\b",
        ],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Celery",
        patterns=[r"\bcelery\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="AsyncIO",
        patterns=[r"\basyncio\b", r"\basync\b", r"асинхрон"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="LLM",
        patterns=[
            r"\bllm\b",
            r"large language model",
            r"языков(?:ая|ые)\s+модел",
            r"\bchatgpt\b",
            r"\bclaude\b",
            r"\bgpt[-\s]?\d",
        ],
        related_skills=["ChatGPT", "AI Interaction", "Prompt Engineering"],
    ),
    SkillDefinition(
        canonical_name="ChatGPT",
        patterns=[
            r"\bchatgpt\b",
            r"\bchat[-\s]?gpt\b",
            r"\bgpt[-\s]?\d",
        ],
        related_skills=["LLM", "AI Interaction", "Prompt Engineering"],
    ),
    SkillDefinition(
        canonical_name="Prompt Engineering",
        patterns=[
            r"prompt engineering",
            r"prompting",
            r"\bprompts?\b",
            r"промптинг",
            r"промпт-инж",
        ],
        related_skills=["AI Interaction", "LLM", "ChatGPT"],
    ),
    SkillDefinition(
        canonical_name="AI Interaction",
        patterns=[
            r"ai interaction",
            r"human[-\s]?ai",
            r"llm tooling",
            r"работ[аы]\s+с\s+(?:ии|ai|llm)",
            r"взаимодейств\w+\s+с\s+(?:ии|ai|llm)",
        ],
        related_skills=["Prompt Engineering", "ChatGPT", "LLM"],
    ),
    SkillDefinition(
        canonical_name="Automation",
        patterns=[
            r"\bautomation\b",
            r"автоматизац",
            r"автоматизирован",
            r"\bworkflow\b",
            r"рабоч(?:ий|ие)\s+процесс",
        ],
        related_skills=["AI Workflow", "No-code", "Automation Tooling"],
    ),
    SkillDefinition(
        canonical_name="AI Workflow",
        patterns=[
            r"ai[-\s]?workflow",
            r"ai[-\s]?assisted\s+workflow",
            r"llm\s+workflow",
            r"ai\s+pipeline",
            r"ии[-\s]?workflow",
            r"ии[-\s]?процесс",
        ],
        related_skills=["Automation", "Automation Tooling", "LLM"],
    ),
    SkillDefinition(
        canonical_name="No-code",
        patterns=[
            r"\bno[-\s]?code\b",
            r"\bnocode\b",
            r"\blow[-\s]?code\b",
            r"без\s+кода",
            r"\bzero[-\s]?code\b",
            r"зерокодинг",
        ],
        related_skills=["Automation", "Automation Tooling"],
    ),
    SkillDefinition(
        canonical_name="Automation Tooling",
        patterns=[
            r"automation tooling",
            r"automation tools?",
            r"инструмент\w+\s+автоматизац",
            r"no[-\s]?code\s+tools?",
        ],
        related_skills=["Automation", "No-code", "AI Workflow"],
    ),
    SkillDefinition(
        canonical_name="Artificial Intelligence",
        patterns=[
            r"\bai\b",
            r"\bии\b",
            r"искусственн\w*\s+интеллект",
            r"ai[-\s]?(?:инструмент|агент|ассистент|систем|решени|анализ|контроль)",
            r"ии[-\s]?(?:инструмент|агент|ассистент|систем|анализ|контроль|решени)",
            r"нейроассистент",
        ],
        related_skills=["LLM", "Neural Networks", "ChatGPT", "Prompt Engineering", "AI Tools", "AI Coding"],
    ),
    SkillDefinition(
        canonical_name="Neural Networks",
        patterns=[r"нейросет", r"neural\s+network"],
        related_skills=["Artificial Intelligence", "LLM", "Machine Learning"],
    ),
    SkillDefinition(
        canonical_name="Chat Bots",
        patterns=[
            r"чат[-\s]?бот",
            r"chat[-\s]?bot",
            r"telegram[-\s]?бот",
            r"telegram\s+bot",
            r"\bботы\b",
            r"\bботов\b",
            r"\bбота\b",
        ],
        related_skills=["Artificial Intelligence", "Automation"],
    ),
    SkillDefinition(
        canonical_name="AI Tools",
        patterns=[
            r"ai[-\s]?инструмент",
            r"ai[-\s]?tools?",
            r"ии[-\s]?инструмент",
            r"оркестратор\w*\s+(?:ai|ии|нейросет)",
        ],
        related_skills=["Artificial Intelligence", "AI Coding", "Automation"],
    ),
    SkillDefinition(
        canonical_name="AI Coding",
        patterns=[
            r"\bcodex\b",
            r"vibe[-\s]?coding",
            r"vibe[-\s]?код",
            r"\bcopilot\b",
            r"\bcursor\b",
            r"ai[-\s]?coding",
        ],
        related_skills=["Artificial Intelligence", "ChatGPT", "Prompt Engineering"],
    ),
    SkillDefinition(
        canonical_name="Data Science",
        patterns=[r"data science", r"data scientist", r"анализ\s+данных"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Machine Learning",
        patterns=[r"machine learning", r"\bml\b", r"машинн(?:ое|ого)\s+обуч"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="TensorFlow",
        patterns=[r"\btensorflow\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="PyTorch",
        patterns=[r"\bpytorch\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Pandas",
        patterns=[r"\bpandas\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="NumPy",
        patterns=[r"\bnumpy\b"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Scikit-learn",
        patterns=[r"scikit-learn", r"sklearn"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="NLP",
        patterns=[r"\bnlp\b", r"обработк[аи]\s+текста"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="RAG",
        patterns=[r"\brag\b", r"retrieval[-\s]?augmented"],
        related_skills=[],
    ),
    SkillDefinition(
        canonical_name="Сантехника",
        patterns=[
            r"сантехник",
            r"сантехническ",
            r"инженерн\w+\s+коммуникац",
            r"инженерн\w+\s+систем",
            r"трубопровод",
            r"запорн\w+\s+арматур",
        ],
        related_skills=[
            "обслуживание инженерных систем",
            "ремонт трубопроводов",
        ],
    ),
]
